...
import { useOverlayPosition } from './useOverlayPosition'; // adjust path
...
//
  ...
  const TabContext = createContext<[SidebarTab | null, (tab: SidebarTab | null) => void]>([null, () => {}]);
  ...
  const [isTransitioningTemplate, setIsTransitioningTemplate] = useState(false);
  ...
  const cardGridGroupRef = useRef<Konva.Layer>(null);

  const [templateReady, setTemplateReady]=useState<boolean>(false);

  const [templateSelected, setTemplateSelected] = useState<boolean>(false);
  
  const [templateRendering, setTemplateRendering] = useState<boolean>(false);
  ...
  templateReady, setTemplateReady,
  templateSelected, setTemplateSelected,
  templateRendering, setTemplateRendering,
  searchParams,
